...
import com.kreezcraft.blockblocker.ConfigHandler;
...
public class BlockBlockerForge {
    ...
    private void foxFire(PlayerInteractEvent.RightClickBlock event) {
        ...
        if (CommonClass.checkMap(CommonClass.interact, blockRegistryName, dimensionLocation) == InteractionResult.FAIL || isBlockedForTeam(player, blockRegistryName)) {
            event.setCanceled(true);
        }
    }

    private boolean isBlockedForTeam(Player player, ResourceLocation blockRegistryName) {
        String blockName = blockRegistryName.toString();
        String teamName = player.getTeam() != null ? player.getTeam().getName() : "no_team";
        List<String> blockedTeams = ConfigHandler.blockedBlocks.get(blockName);
        return blockedTeams != null && blockedTeams.contains(teamName);
    }
}
